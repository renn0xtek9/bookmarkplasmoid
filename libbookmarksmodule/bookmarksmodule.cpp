...
bool Bookmarkmodule::read(QIODevice *device)
{
	m_bookmarks.clear();
...
QString Bookmarkmodule::errorString() const
{
	return QObject::tr("%1\nLine %2, column %3")
		.arg(xml.errorString())
		.arg(xml.lineNumber())
...
void Bookmarkmodule::readFolder()
{
...
void Bookmarkmodule::readInfoAndMetadata(QString p_blockname)
{
...
void Bookmarkmodule::readBookmark()
{
...
QString Bookmarkmodule::readTitle()
{
...
void Bookmarkmodule::readSeparator()
{
	Q_ASSERT(xml.isStartElement() && xml.name() == "separator");
	xml.skipCurrentElement();
}
QList<Bookmark> Bookmarkmodule::getBookrmarks()
{
	return m_bookmarks;
}
